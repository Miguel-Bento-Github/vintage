...
export default function ShopClient({ initialProducts }: ShopClientProps) {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (selectedSizes.length > 0) {
      filtered = filtered.filter(
        (p) => p.size && selectedSizes.includes(p.size.label)
      );
    }
    ...
    if (selectedConditions.length > 0) {
      filtered = filtered.filter((p) =>
        selectedConditions.some(
          (cond) => cond.toLowerCase() === p.condition.toLowerCase()
        )
      );
    }
  ...
  const availableCategories = useMemo(() => {
    ...
    return Array.from(
      new Set(selectedProductTypes.flatMap((type) => CATEGORIES_BY_TYPE[type]))
    );
  }, [selectedProductTypes]);
    ...
    <div className="min-h-screen relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
        <div className="mb-8">
          ...
          <p className="text-gray-600">{t("browseCollection")}</p>
        </div>
